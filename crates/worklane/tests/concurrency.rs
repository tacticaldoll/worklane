...
/// Build a ready worker over `broker` with the given concurrency and a `GateJob`
/// wired to `c`.
fn worker_with(
...
/// `run_until_idle` honours the configured concurrency, not just `run`. With
/// concurrency 3 and three jobs, all three run at once. Under a sequential drain
/// this would dead-lock: the first handler blocks on a gate that only opens once
/// all three are in flight, which never happens if jobs run one at a time.
#[tokio::test]
async fn run_until_idle_honors_concurrency() {
    let broker = Arc::new(InMemoryBroker::new());
    let client = Client::new(broker.clone());
    let c = counters();
    let worker = worker_with(broker.clone(), &c, 3);
    ...
    let handle = tokio::spawn(async move { drain_worker.run_until_idle().await });

    // All three reach the gate concurrently — impossible under a sequential drain.
    spin_until(|| c.current.load(SeqCst) == 3).await;
    assert_eq!(c.max.load(SeqCst), 3, "three handlers ran at once");
    c.gate.add_permits(3);

    timeout(Duration::from_secs(5), handle)
        .await
        .expect("run_until_idle returns once drained")
        .expect("task join")
        .expect("run_until_idle result");
    assert_eq!(broker.len(), 0, "every job processed");
    assert_eq!(c.runs.load(SeqCst), 3);
}
