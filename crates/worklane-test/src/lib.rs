...
//! [`for_each_required_scenario`] / [`for_each_timed_scenario`] /
//! [`for_each_configured_scenario`] drivers — so a scenario can never be
//! silently dropped from one backend's hand-maintained list. A backend supplies
//! an *emitter* macro that turns each name into a
...
//! - [`for_each_required_scenario`] — every broker (time-free).
...
/// The single source of truth for the time-free broker-contract scenario set.
///
...
#[macro_export]
macro_rules! for_each_required_scenario {
    ($cb:ident) => {
        $cb! {
...
/// `scenarios::<name>` against it. See [`for_each_required_scenario`] for the
/// callback contract.
#[macro_export]
...
/// advance their injected clock invoke this. See [`for_each_required_scenario`]
