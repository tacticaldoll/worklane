...
continuation, parallel groups, and chord aggregations via persistent result
stores and live-window unique keys.

## Requirements

### Requirement: Idempotent Continuation (Chain)
...
`chain:{parent_id}:{next_kind}`, to provide live-window deduplication, and by
...
The chord callback enqueued under the stable `chord:{chord_id}:callback` key
...
#### Scenario: Firing chord callback

- **WHEN** a watcher job generation calls `build_continuation_keyed` with a
  stable `chord:{chord_id}:callback` key and enqueues it
...
### Requirement: Chord Watcher Self-Rescheduling

The system SHALL implement Chord aggregation using a self-rescheduling
`ChordWatcherJob` that captures each dependency's output from a `ResultStore`
...
the watcher payload for the remainder of the chord's lifetime, and subsequent
generations SHALL poll only the dependencies not yet captured. A result-store
eviction of an already-captured dependency's result MUST NOT regress the chord.
...
- Else if the broker classifies it `DeadLettered`, the watcher SHALL fail the
  chord promptly with an error naming the failed dependency.
...
- If the completed result is absent, the dependency completed but its result was
  evicted before capture; the watcher SHALL fail the chord with an error naming
  the dependency.

When every dependency's value has been captured, the watcher SHALL build a
`ChordResults<C>` payload, containing the caller's context `C` plus each
dependency's opaque output bytes in dependency order, and enqueue the callback
under a stable `chord:{chord_id}:callback` key. The callback is therefore a
...
- **WHEN** a chord watcher job runs and some dependency's value is not yet
  captured
...
- **WHEN** a chord watcher job runs and every dependency's value is captured
- **THEN** it builds `ChordResults<C>` and enqueues the callback under the stable
  callback key
- **AND** it acks itself

#### Scenario: ChordResults wire shape round-trips

- **WHEN** the watcher builds the callback payload without knowing the caller's
  context type
- **THEN** the payload SHALL deserialize as `ChordResults<C>` for the callback's
  context type
...
- **AND** the chord SHALL still complete and deliver that value once the
  remaining dependencies are captured

#### Scenario: Dead-lettered dependency fails the chord fast

- **WHEN** a dependency exhausts its retries and is dead-lettered
- **THEN** the watcher SHALL classify it `DeadLettered` and fail the chord
  promptly
- **AND** the failure SHALL name the dead-lettered dependency

#### Scenario: A result evicted before capture fails the chord
...
- **WHEN** a dependency completed but its result is absent from the `ResultStore`
  ...
  `CompletedOrUnknown`
- **THEN** the watcher SHALL fail the chord with an error naming the dependency
...
### Requirement: Chord dependency identity is causally guaranteed

A chord submitted through the facade `Client::chord` SHALL guarantee that every
dependency id recorded in the watcher payload denotes a job that was actually
persisted. To this end, `chord` SHALL reject any dependency that carries a
...
is causally sound: for a dependency originating from `chord`, `CompletedOrUnknown`
can only mean the job was acked, never that it was never enqueued.

#### Scenario: A dependency carrying a unique_key is rejected

- **WHEN** `Client::chord` is called with a dependency that carries a `unique_key`
...
- **WHEN** `Client::chord` is called with dependencies that carry no `unique_key`
...
### Requirement: Chord Watcher Payload Validation

`ChordWatcherPayload` is a serialized internal job payload and MUST NOT be
