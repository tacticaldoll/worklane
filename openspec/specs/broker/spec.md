...
accepts multiple jobs and MUST ensure all-or-nothing insertion. A broker MAY
omit the capability, in which case its `batch_enqueue` accessor SHALL return
absent and a batch operation SHALL fail predictably with an
unsupported-capability result rather than silently degrading.
