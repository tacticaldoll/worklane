# Broker Extensibility Specification

## Purpose

Defines the public broker-author extension model: the broker-author SPI surface
(`worklane_core::spi`), the modular conformance suites a broker proves itself
against, and how a custom broker states a compatibility claim. It builds on the
`broker` capability's minimal lifecycle contract and optional-capability
accessors, making the verified-lifecycle guarantee legible and reproducible for
private and third-party broker authors rather than only first-party crates.
