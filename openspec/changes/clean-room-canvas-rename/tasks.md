...
- [ ] 1.1 Rename the source module `crates/worklane/src/canvas.rs` →
      `workflow.rs`; update `mod canvas` → `mod workflow` and every
      `crate::canvas::` → `crate::workflow::` (lib.rs, client.rs, client_builder.rs).
- [ ] 1.2 Rename public types: `Canvas`→`Workflow`, `ChordResults`→`FanInResults`,
...
- [ ] 2.4 Update incidental references in `crates/worklane-test/src/scenarios/
      unique.rs` (the `chord:`-style example key) and the comment in
      `crates/worklane-redis/src/lib.rs`.

## 3. Specs

- [ ] 3.1 Sync the terminology delta into `openspec/specs/workflow-canvas/spec.md`
      (restate every requirement in fan-in/fan-out vocabulary; behavior
      unchanged).
- [ ] 3.2 Rename the capability: `git mv openspec/specs/workflow-canvas
      openspec/specs/workflow` and change the spec title to `# Workflow` (final
      post-sync step, per design.md).
- [ ] 3.3 Fix incidental non-normative references in
...
- [ ] 4.1 Remove Celery from `README.md` (the "Celery, via Kombu" paragraph, the
      diagram caption, and "not a Celery clone") and `docs/architecture.md`; the
      technical points stand without it.
- [ ] 4.2 Remove/neutralize the Celery line in `openspec/config.yaml` ("Inspired
      by Celery … do NOT treat it as a Celery clone") and rename "workflow canvas
      (chains / chords)" to fan-in/fan-out vocabulary.
- [ ] 4.3 Rewrite `docs/design/workflow-canvas.md` to fan-in/fan-out vocabulary
      (title, topology names, type names, keys).
- [ ] 4.4 Update `AGENTS.md` (the "client, canvas" facade-module mention →
      "workflow").
- [ ] 4.5 Fix any Celery/chord references in `BACKLOG.md`.

## 5. Version + changelog

- [ ] 5.1 Bump the workspace version `0.1.0` → `0.2.0` in `Cargo.toml`
      (`[workspace.package]` and internal `[workspace.dependencies]` pins).
- [ ] 5.2 Add a `CHANGELOG.md` 0.2.0 entry documenting the breaking rename with
      the migration map (do not rewrite older entries).

## 6. Boundaries (must hold)

- [ ] 6.1 No behavior change: every `workflow` capability WHEN/THEN is preserved;
      the delta is vocabulary-only.
- [ ] 6.2 No `Broker`/`worklane-core` change; the rename is confined to the
      `worklane` facade, specs, and docs.
- [ ] 6.3 No perf/worker change is folded in.
- [ ] 6.4 Live-surface grep gate: no stray `Celery`, `Chord`, `Canvas`, `chord`,
...
- [ ] 7.1 Definition of Done: `cargo build`, `cargo test`, `cargo clippy
      --all-targets -- -D warnings`, `cargo fmt --all --check` all pass.
- [ ] 7.2 Full conformance: run the Postgres + Redis conformance suites against
